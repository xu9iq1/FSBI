...
class SBI_Dataset(Dataset):
	...
	def __getitem__(self,idx):
		...
		# 	return self.get_dwt(img_f),self.get_dwt(img_r)
	...
	def get_dwt(self, img):
		...
		cH=cv2.resize(cH,self.image_size,interpolation=cv2.INTER_LINEAR).astype('float32')
		cV=cv2.resize(cV,self.image_size,interpolation=cv2.INTER_LINEAR).astype('float32')
