...
class ESBI_Dataset(Dataset):
	def __init__(self,phase='train',image_size=224,n_frames=8, wavelet="sym2", mode="reflect"):
	...
	def __getitem__(self,idx):
		...
		# if np.random.rand()<0.5:
		return self.get_dwt_rgb(img_f),self.get_dwt_rgb(img_r)

		return img_f.transpose((2,0,1)),img_r.transpose((2,0,1))

	def get_dwt_rgb(self, img):
		b, g, r = cv2.split(img)

		cA_r, (cH_r, cV_r, cD_r) = pywt.dwt2(r, self.w, mode=self.m)
		cA_g, (cH_g, cV_g, cD_g) = pywt.dwt2(g, self.w, mode=self.m)
		cA_b, (cH_b, cV_b, cD_b) = pywt.dwt2(b, self.w, mode=self.m)

		cA_r = cv2.resize(cA_r, self.image_size, interpolation=cv2.INTER_LINEAR).astype('float32')
		cA_g = cv2.resize(cA_g, self.image_size, interpolation=cv2.INTER_LINEAR).astype('float32')
		cA_b = cv2.resize(cA_b, self.image_size, interpolation=cv2.INTER_LINEAR).astype('float32')
	...
	def get_dwt(self, img):
		img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

		cA, (cH, cV, cD) = pywt.dwt2(img, 'sym2', mode='symmetric')
		cA=cv2.resize(cA,self.image_size,interpolation=cv2.INTER_LINEAR).astype('float32')

		cA = (cA + img)/2
		img_dwt = [cA, cA, cA]

		img_dwt = np.array(img_dwt)

		return img_dwt

	def get_gabor_texture(self, img, frequency=0.6):
		gray_image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
		gabor_response = filters.gabor(gray_image, frequency=frequency)[0]

		res = np.array([gabor_response, gabor_response, gabor_response])
		return res
